...
MX_SRV="${SYNAPSE_SERVER_NAME:-localhost}"
...
SYNAPSE="${SYNAPSE_URL:-http://127.0.0.1:${SYNAPSE_PORT:-8008}}"
...
LOGIN="$(curl -sf -X POST "${SYNAPSE}/_matrix/client/v3/login" \
  ...
  -d "$LOGIN_PAYLOAD")" || {
  echo "e2e-matrix-mentions: Matrix 登录失败（Synapse 是否在本机 ${SYNAPSE_PORT:-8008} 监听？）" >&2
  exit 2
}
...
if [[ -z "$PASS_JSON" ]]; then
  ...
  echo "e2e-matrix-mentions: 检查 LLM、openclaw 日志；SYNAPSE_SERVER_NAME=${MX_SRV}" >&2
  exit 4
fi
