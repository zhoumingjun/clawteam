...
- **务必优先写完整 Matrix ID**：`@角色:localhost`（与 `.env` 里 `MATRIX_SERVER_NAME` 一致；若生产改成别的域名，把 `:localhost` 换成实际 server 名）。这样客户端/网关才能产生 **`m.mentions`**，对方能收到通知；团队房通常 **`requireMention: true`**，点名不规范会没人接单。
...
- **禁止公开发言**：在团队房内发出的每条消息都必须带有 **至少一名** 具体收件人的 **完整 Matrix ID**（`@localpart:域名`，域名同 `MATRIX_SERVER_NAME`）。不得发送未点名任何人的「广播」、闲聊或自言自语式内容；没有明确收件人就不要发。
...
- **标题**: {简短描述}
- **描述**: {详细说明}
- **负责人**: @dev:localhost（须写 **完整 Matrix ID**：`@localpart:域名`，与 `MATRIX_SERVER_NAME` 一致）
- **优先级**: P0/P1/P2
